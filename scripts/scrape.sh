...
# Note: run this ONLY with enough memory
...
echo "$(date): Making a backup of $POSTS_FILE..."
cp "$POSTS_FILE" "$POSTS_FILE.$(date +%Y%m%d%H%M%S).bak"
...
echo "$(date): Loading all subreddit feather files into $POSTS_FILE"
python -m src.slow.reddit.make "$SUBREDDIT_DIR"/*.feather "$POSTS_FILE" --update --verbose --maxops 1
...
while python -m src.slow.reddit.make "$POSTS_FILE" --update --verbose --maxops $POSTS_BATCH
do
    echo "$(date) Iteration completed"
done
